...
LIB8STATIC uint16_t triwave16(uint16_t in)
{
    if( in & 0x8000) {
        in = 0xffff - in;
    }
...
LIB8STATIC uint16_t squarewave16( uint16_t in, uint16_t pulsewidth=0x8000)
{
    if( in < pulsewidth || (pulsewidth == 0xffff)) {
        return 0xffff;
    } else {
        return 0;
    }
...
#else
uint32_t get_millisecond_timer() {return time_us_32() / 1000;}
...
typedef CEveryNTimePeriods<unsigned long,GET_MILLIS> CEveryNMillis;
